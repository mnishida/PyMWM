...
class Cutoff():
    ...
    def __call__(self, alpha, r_ratio):
        x = self.samples.query(
            f"pol=='{alpha[0]}' and n=={alpha[1]} and m=={alpha[2]}")['rr']
        y = self.samples.query(
            f"pol=='{alpha[0]}' and n=={alpha[1]} and m=={alpha[2]}")['val']
        x = x.to_numpy()
        y = y.to_numpy()
    ...
    def PEC(self, x, r_ratio, n, pol):
        ...
        if pol == 'E':
            f = jpv * ypx - ypv * jpx
            fp = (v * jppv * ypx + x * jpv * yppx -
                  v * yppv * jpx - x * ypv * jppx)
        else:
            f = jv * yx - yv * jx
            fp = (v * jpv * yx + x * jv * ypx -
                  v * ypv * jx - x * yv * jpx)
        return f, fp
                ...
                for n in range(num_n):
                    for m in range(1, num_m+1):
                        if r_ratio == self.r_ratios[1]:
                            ...
                            ini_val = (z[-1][(pol, n, m)] * 2 -
                                       z[-2][(pol, n, m)])
                        sol = so.root_scalar(
                            self.PEC, x0=ini_val, fprime=True,
                            args=(r_ratio, n, pol))
                        kini = sol.root
                        data[(pol, n, m)] = kini
            z.append(data)
            ...
            for pol in ['E', 'M']:
                for n in range(self.num_n):
                    for m in range(1, self.num_m + 1):
                        df = df.append(
                            {'irr': i, 'rr': r_ratio, 'pol': pol, 'n': n,
                             'm': m, 'val': sample[(pol, n, m)]},
                            ignore_index=True)
    ...
    def cutoffs(self):
        ...
        def func(alpha, kini, rrs):
            ...
            z = [kini]
            sol = so.root_scalar(
                self.PEC, x0=kini, fprime=True, args=(rrs[1], n, pol))
            z = [kini, sol.root]
