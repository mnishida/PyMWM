...
@cython.cdivision(True)
def eig_eq_u(
    double[::1] uvec, cdouble w, str pol, int n, cdouble e1, cdouble e2, double r, double ri, cdouble[::1] roots
) -> np.ndarray:
    """Return the value of the characteristic equation

    Args:
        uvec: The real and imaginary parts of u.
        w: The angular frequency
        pol: The polarization
        n: The order of the modes
        e1: The permittivity of the core
        e2: The permittivity of the clad.
    Returns:
        f.real, f.imag:  The left-hand value of the characteristic equation.
    """
    cdef:
        cdouble u = uvec[0] + uvec[1] * 1j
        # cdouble hu = hankel2(n, u)
        cdouble[:, ::1] a = np.empty((4, 4), dtype=complex)
        cdouble f, denom
        int i
        int num = len(roots)

    eig_mat_u(u, w, pol, n, e1, e2, r, ri, a)
    if n == 0:
        if pol == "E":
            f = a[0, 0] * a[1, 1] - a[0, 1] * a[1, 0]
        else:
...
def eig_eq_u_for_min(
    ...
    Args:
        ...
        e2: The permittivity of the clad.
    Returns:
        f:  The left-hand value of the characteristic equation.
    """
    cdef:
        cdouble u = uvec[0] + uvec[1] * 1j
        # cdouble hu = hankel2(n, u)
...
cdef void eig_mat_u_with_deriv(
    ...
    for i in range(4):
        for j in range(4):
            b[i, j] = (da_du[i][j] + da_dv[i][j] * dv_du
                        + da_dx[i][j] * dx_du + da_dy[i][j] * dy_du)
    b[2, 0] += 1.0 / (e2 * w2) * nuv * ju * kv * dh2_du
    b[2, 1] += 1.0 / (e2 * w2) * nuv * yu * kv * dh2_du
    b[3, 0] += 1.0 / (e2 * w2) * nxy * jx / yx * dh2_du
    b[3, 1] += 1.0 / (e2 * w2) * nxy * dh2_du
...
def eig_eq_u_with_jac(
    ...
    eig_mat_u_with_deriv(u, w, pol, n, e1, e2, r, ri, a, b)
    if n == 0:
        if pol == "E":
            f = a[0, 0] * a[1, 1] - a[0, 1] * a[1, 0]
            fp = deriv_det2(
    ...
    for i in range(num):
        denom *= (u - roots[i]) / roots[i]
        ddi = - roots[i] / (u - roots[i]) ** 2
        for j in range(num):
            if j != i:
                ddi /= (u - roots[j]) / roots[j]
        dd += ddi
    fp = fp / denom + f * dd
    f /= denom
    return np.array([f.real, f.imag]), np.array([[fp.real, fp.imag], [-fp.imag, fp.real]])
